...
import { confirmLeaseMoveOutDeparture } from "../../../../../../api/leases/move-out";
...
export async function POST(
  ...
  try {
    const data = await confirmLeaseMoveOutDeparture(
      lease,
    ...
    if (error instanceof Error && error.message === "MOVE_OUT_NOT_PLANNED") {
      return jsonResponse(409, {
        ...
        error: "Aucune fin de location planifiée à confirmer."
      });
    }
    console.error("Failed to confirm move-out departure", error);
    return jsonResponse(500, {
      success: false,
      code: "INTERNAL_ERROR",
      error: "Impossible de confirmer le départ"
    });
  }
}
