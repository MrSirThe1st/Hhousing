...
export async function GET(
  ...
  if (!paymentId?.trim()) {
    return jsonResponse(
      400,
      { success: false, code: "VALIDATION_ERROR", error: "Payment id is required" },
      request
    );
  }
  ...
  try {
    const payments = await paymentRepo.listPaymentsByTenantAuthUserId(
      access.data.userId,
    ...
    console.error("Failed to fetch tenant payment detail", error);
    return jsonResponse(
      500,
      {
        success: false,
