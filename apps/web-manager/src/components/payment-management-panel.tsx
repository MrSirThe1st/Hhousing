...
import type { Payment, PaymentStatus } from "@hhousing/domain";
...
export default function PaymentManagementPanel({
  ...
  async function handleGenerateCharges(): Promise<void> {
    ...
    setMessage(
      generated === 0
        ? `Aucune nouvelle charge générée pour ${period} (déjà générées ou aucun bail actif).`
        : `${generated} charge(s) de loyer générée(s) pour ${period}.`
    );
    setGenerating(false);
    router.refresh();
  }
