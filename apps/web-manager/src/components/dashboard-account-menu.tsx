...
type DashboardAccountMenuProps = {
  ...
  /** Compact trigger for tight headers / mobile drawers. */
  compact?: boolean;
};

export default function DashboardAccountMenu({
  organizationProfileHref = "/dashboard/profile?tab=organisation",
  compact = false
}: DashboardAccountMenuProps): React.ReactElement {
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        className={`inline-flex shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:border-slate-300 hover:text-slate-900 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:border-slate-600 dark:hover:text-white ${
      ...
      {open ? (
        ...
        >
          <Link
            ...
            className="flex items-center gap-2.5 px-3.5 py-2.5 text-sm text-slate-700 transition hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800/70"
          >
            <svg viewBox="0 0 24 24" fill="none" className="h-4 w-4 shrink-0 text-slate-500" aria-hidden="true">
              <path
                d="M4.5 19.5V8.8L12 4.5l7.5 4.3v10.7"
                stroke="currentColor"
                strokeWidth="1.8"
                strokeLinejoin="round"
              />
              <path d="M9.5 19.5v-5h5v5" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round" />
            </svg>
            Organisation
          </Link>
